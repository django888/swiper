#!/usr/bin/env python   该行为声明,证明是用什么程序执行的
...
sys.path.insert(0, BASE_DIR)#这是一个路径变量
...
def create_robots(n):
    ...
    for i in range(n):
        ...
        try:
            User.objects.create(
                phonenum='%s' % random.randrange(81000000000, 81900000000),
                nickname=name,
                sex=int(sex),
                birth_year=random.randint(1980, 2000),
                birth_month=random.randint(1, 12),
                birth_day=random.randint(1, 28),
                location=random.choice(['gz', 'bj', 'sz', 'hz', 'cd', 'sh']),
            )
